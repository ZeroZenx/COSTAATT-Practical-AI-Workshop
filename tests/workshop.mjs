...
await check('Landing page, logo asset, direct entry and registration validation',async()=>{
 await go('');assert.equal(await page.title(),'Practical AI for Professionals · Workshop');assert(await page.locator('.costaatt-logo').evaluate(el=>el.complete&&el.naturalWidth===474));
