...
await check('Landing page, logo asset, direct entry and registration validation',async()=>{
 await go('');assert.equal(await page.title(),'Practical AI for Professionals · Workshop');assert(await page.locator('.costaatt-logo').evaluate(el=>el.complete&&el.naturalWidth===474));assert.equal(await page.locator('.qr-frame img').count(),1);assert(await page.locator('.qr-frame img').evaluate(el=>el.complete&&el.naturalWidth>0));assert.equal(await page.locator('.join-url').getAttribute('href'),'https://costaatt-ai-workshop.darren-headley.chatgpt.site/#join');
