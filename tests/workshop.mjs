...
await check('Remote projector link displays live state and uses facilitator-code control',async()=>{
 ...
 assert.equal((await fetch(base+'api/projection/rooms',{method:'POST',headers:{'content-type':'application/json','oai-authenticated-user-id':'test-owner'},body:JSON.stringify(initial)})).status,401);const created=await fetch(base+'api/projection/rooms',{method:'POST',headers:{'content-type':'application/json','x-workshop-control-code':'TEST-CODE-2226'},body:JSON.stringify(initial)});assert.equal(created.status,201);const {roomId}=await created.json();
 ...
 const next={projection:{session:5,activity:0,mode:'reveal'},timer:null};const changed=await fetch(base+'api/projection/rooms/'+roomId,{method:'PUT',headers:{'content-type':'application/json','x-workshop-control-code':'TESTCODE2226'},body:JSON.stringify(next)});assert.equal(changed.status,200);await projector.getByRole('heading',{name:'A useful approach.'}).waitFor();assert.match(await projector.locator('.projection-header').textContent(),/SESSION 06 \/ 08/);await projector.close();
 assert.equal((await fetch(base+'api/projection/rooms/'+roomId,{method:'DELETE'})).status,401);assert.equal((await fetch(base+'api/projection/rooms/'+roomId,{method:'DELETE',headers:{'x-workshop-control-code':'TESTCODE2226'}})).status,204);
 const adminContext=await browser.newContext();await adminContext.route('**/api/projection/rooms**',route=>route.continue({headers:{...route.request().headers(),'x-workshop-control-code':'TESTCODE2226'}}));const admin=await adminContext.newPage();await admin.goto(base+'#facilitator');await admin.locator('#facilitator-access-code').fill('TEST-CODE-2226');await admin.getByRole('button',{name:'Start remote screen'}).click();await admin.locator('#projection-room-link').waitFor();assert.match(await admin.locator('#projection-room-link').inputValue(),/#present\?room=/);await adminContext.close();
});
await check('Participant report unlocks with facilitator code and hides details until expanded',async()=>{
 await go('#reporting');await page.getByRole('button',{name:'Unlock report'}).waitFor();assert.match(await page.locator('#reporting-status').textContent(),/Enter the facilitator code/);assert.equal(await page.getByRole('link',{name:'Sign in with ChatGPT'}).count(),0);await page.locator('#facilitator-access-code').fill('TEST-CODE-2226');await page.getByRole('button',{name:'Unlock report'}).click();await page.getByText(/loaded\.|No shared progress yet\./).waitFor();assert.equal(await page.locator('.reporting-details').evaluate(el=>el.open),false);assert.match(await page.locator('.reporting-retention').textContent(),/Clear shared records/);
});
